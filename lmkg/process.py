...
def process_matrix(
    attentions, layer_idx=-1, head_num=0, avg_head=False, trim=True, use_cuda=True
):
    if avg_head:
        if use_cuda:
...
def filter_relation_sets(params):
    ...
    if head in id2token and tail in id2token:
        ...
        tail = id2token[tail]
        relations = [
            spacy_nlp(id2token[idx])[0].lemma_
        ...
        for data in tqdm(dataset["data"], dynamic_ncols=True):
            for para in data["paragraphs"]:
                ...
                for sent in nlp(context).sents:
                    for output in parse_sentence(
                        sent.text, tokenizer, encoder, nlp, use_cuda=use_cuda
                    ):
                ...
                for question in para["qas"]:
                    ...
                    for output in parse_sentence(
                        question, tokenizer, encoder, nlp, use_cuda=use_cuda
                    ):
                        f.write(json.dumps(output) + "\n")
                f.flush()
        f.close()
